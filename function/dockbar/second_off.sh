...
grep "Clock" ~/.config/tint2/dock_tint2rc; 
  ...
  then
   ...
   if [ "$?" == "1" ];
    then
     ...
     grep "%I" ~/.config/tint2/dock_tint2rc;  
     if [ "$?" == "1" ];
      then
       sed -i "s/time1_format = %H:%M:%S/time1_format = %H:%M/g" ~/.config/tint2/dock_tint2rc;
       killall -SIGUSR1 tint2;
      else
       sed -i "s/time1_format = %I:%M:%S/time1_format = %I:%M/g" ~/.config/tint2/dock_tint2rc;
       killall -SIGUSR1 tint2;
    fi;
   fi;
fi;
